...
package org.springframework.cloud.stream.pubsub.bridge;
...
class PubSubBridgeEndToEndIntegrationTests {
	...
	void sourceIntegrationTest() {
		try (ConfigurableApplicationContext context = new SpringApplicationBuilder(AdditionalConfig.class)
			.run(
				"--server.port=0",
				"--spring.cloud.stream.pubsub.bridge.running-mode=PUBLISHER",
				"--spring.cloud.stream.pubsub.bridge.publisher.destination=foobar")) {
	...
	void sinkIntegrationTest() throws InterruptedException {
		try (ConfigurableApplicationContext context = new SpringApplicationBuilder(AdditionalConfig.class, TestController.class)
			.run(
				"--server.port=8084",
				"--spring.cloud.stream.pubsub.bridge.running-mode=SUBSCRIBER",
				"--spring.cloud.stream.pubsub.bridge.subscriber.destination=foobar",
				"--spring.cloud.stream.pubsub.bridge.subscriber.invokable-endpoint=http://localhost:8084/passthrough")) {
	...
	void sinkIntegrationTestPojo() throws Exception {
		try (ConfigurableApplicationContext context = new SpringApplicationBuilder(AdditionalConfig.class, TestController.class)
			.run(
				...
				"--spring.cloud.stream.pubsub.bridge.subscriber.invokable-endpoint=http://localhost:8084/passthrough-pojo")) {
			var inputDestination = context.getBean(InputDestination.class);
			var testPojo = new TestPojo();
			testPojo.setName("foobar");
