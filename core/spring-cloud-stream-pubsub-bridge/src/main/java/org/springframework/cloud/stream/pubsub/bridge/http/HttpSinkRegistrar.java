...
package org.springframework.cloud.stream.pubsub.bridge.http;
...
import org.springframework.cloud.stream.pubsub.bridge.core.AbstractFunctionsRegistrar;
...
public class HttpSinkRegistrar extends AbstractFunctionsRegistrar {
	...
	private final PubSubBridgeProperties pubSubBridgeProperties;
	...
	public HttpSinkRegistrar(FunctionCatalog functionCatalog, Consumer<byte[]> proxyConsumer, ConfigurableEnvironment configurableEnvironment,
							 PubSubBridgeProperties pubSubBridgeProperties, StreamFunctionProperties streamFunctionProperties,
							 BindingServiceProperties bindingServiceProperties) {
		this.functionCatalog = functionCatalog;
		this.proxyConsumer = proxyConsumer;
		this.configurableEnvironment = configurableEnvironment;
		this.pubSubBridgeProperties = pubSubBridgeProperties;
	...
	public void afterPropertiesSet() {
		if (isSubscriber()) {
			...
			String subscriptionDestination = this.pubSubBridgeProperties.getSubscriber().getDestination();
			...
			String sendToDestination = this.pubSubBridgeProperties.getSubscriber().getSendToDestination();
	...
	private boolean isSubscriber() {
		PubSubBridgeProperties.RunningMode runningMode = this.pubSubBridgeProperties.getRunningMode();
		return runningMode == PubSubBridgeProperties.RunningMode.SUBSCRIBER ||
			runningMode == PubSubBridgeProperties.RunningMode.SUBSCRIBER_PUBLISHER;
	}
